...
class AnnoRect(object):
    ...
    def writeIDL(self, file):
        file.write(" ("+str(self.x1)+", "+str(self.y1) +
                   ", "+str(self.x2)+", "+str(self.y2)+")")
        if (self.score != -1):
    ...
    def overlap_pascal(self, other):
        ...
        union = self.width() * self.height() + other.width() * \
            other.height() - interSection
...
class Annotation(object):
    ...
    def writeIDL(self, file):
        if (self.frameNr == -1):
            ...
            file.write("\""+os.path.join(self.imagePath,
                                         self.imageName)+"@%d\"" % self.frameNr)
        ...
        if (posImageEnd != -1 and posImageEnd+4 < lineLen):
            ...
            if (len(segments) % 4 != 0):
                ...
                for i in range(0, len(segments), 4):
                    rectSeg = segments[i]+","+segments[i+1] + \
...
def getStats(annotations):
    ...
    print("     avg. Width:", avgWidth, " (", sqrt(
        varWidth), "standard deviation )")
    print("    avg. Height:", avgHeight, " (", sqrt(
        varHeight), "standard deviation )")

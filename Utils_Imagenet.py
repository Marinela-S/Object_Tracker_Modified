...
def run_inception_once(picture_path):
    ...
    with tf.Session() as sess:
        ...
        # CHECK BEST LABEL
        print("Best Label: %s with conf: %.5f" %
              (label_lines[top_k[0]], predictions[top_k[0]]))
...
def label_video(video_info, frames):
    ...
    if not os.path.exists(frames[0].split("/")[0]+"/cropped_rects/"):
        ...
        print("Created Folder: %s" %
              (frames[0].split("/")[0]+"/cropped_rects/"))
    # Loads label file, strips off carriage return
    label_lines = [line.rstrip() for line in tf.gfile.GFile(label_file)]
    # Creates graph from saved GraphDef.
    create_graph()
    ...
    with tf.Session() as sess:
        ...
        for frame_info in progress(video_info):
            ...
            for rect in frame_info.rects:
                ...
                x1, y1, x2, y2 = utils_image.get_orig_rect(
                    width, height, 640, 480, rect.x1, rect.y1, rect.x2, rect.y2)
                print(x1, y1, x2, y2)
                if(x1 == x2):
                    x2 = x2-10
                if(y1 == y2):
                ...
                cropped_img_name = frames[0].split(
                    "/")[0]+"/cropped_rects/cropped_frame_%d_rect_%d.JPEG" % (frame_info.frame, rect_id)
                cropped_img.save(cropped_img_name)
                print("Frame: %d Rect: %d conf: %.2f" %
                      (frame_info.frame, rect_id, rect.true_confidence))
                ...
                # CHECK BEST LABEL
                print("Best Label: %s with conf: %.5f" % (vid_classes.code_to_class_string(
                    label_lines[top_k[0]]), predictions[top_k[0]]))
                rect.set_rect_coordinates(x1, x2, y1, y2)
                rect.set_label(predictions[top_k[0]], vid_classes.code_to_class_string(
                    label_lines[top_k[0]]), top_k[0], label_lines[top_k[0]])
                rect_id = rect_id+1
            idx = idx+1
    return video_info
    ...
    with tf.Session() as sess:
        ...
        for frame_info in progress(video_info):
            ...
            for rect in frame_info.rects:
                ...
                print(cor)
                cropped_img = img.crop(cor)
                cropped_img_name = folder + \
                    "/cropped_rects/cropped_frame_%d_rect_%d.JPEG" % (
                        frame_info.frame, rect_id)
                cropped_img.save(cropped_img_name)
                print("Frame: %d Rect: %d conf: %.2f" % (
                    frame_info.frame, rect_id, rect.true_confidence))
                if not tf.gfile.Exists(cropped_img_name):
                ...
                if(len(video_labels) > 0):
                    if(video_labels[idx-1][rect_id][0] == top_k[0]):
                        # CHECK BEST LABEL
                        print("Best Label: %s with conf: %.5f" % (
                            vid_classes.code_to_class_string(label_lines[top_k[0]]), predictions[top_k[0]]))
                        rect.set_rect_coordinates(x1, x2, y1, y2)
                        rect.set_label(predictions[top_k[0]], vid_classes.code_to_class_string(
                            label_lines[top_k[0]]), top_k[0], label_lines[top_k[0]])
                        ...
                        print("Best Label setted recurrently: %s " % (
                            vid_classes.code_to_class_string(label_lines[label])))
                        rect.set_rect_coordinates(x1, x2, y1, y2)
                        rect.set_label(video_labels[idx-1][rect_id][1], vid_classes.code_to_class_string(
                            label_lines[label]), label, label_lines[label])
                        frame_labels.append(
                            (label, video_labels[idx-1][rect_id][1]))
                else:
                    # CHECK BEST LABEL
                    print("Best Label: %s with conf: %.5f" % (
                        vid_classes.code_to_class_string(label_lines[top_k[0]]), predictions[top_k[0]]))
